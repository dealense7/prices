<div class="relative h-full flex items-center" wire:ignore x-data="{
                update: false,
                options: {{$categories->toJson()}},
                selectedOptions: {{$product->categories->toJson()}},
                open: false,
                filter: ''
                }">
    <div
    ...
    <button :class="{'bg-blue-600 text-white' : update}"
            x-on:click="() => {
                fetch(
                    'api/product/{{$product->id}}',
                    {
                     ...
                     body: JSON.stringify({ categories: selectedOptions.map(obj => obj.id) }) // Fix the syntax error here
                    }
                )
