...
@section('content')
    ...
    <div class="grid grid-cols-10" x-data="{open: ''}">
        <div class="col-span-10 bg-white shadow p-3 rounded-[4px]">
            <ul class="font-medium  text-xs flex items-center justify-between">
                @foreach($categories as $category)
                    <li class="cursor-pointer" x-on:click="open = '{{$category->slug}}'">{{$category->name}}</li>
                @endforeach
            </ul>
        </div>
        @foreach($categories as $category)
            ...
            >
                <ul class="font-normal text-xs flex items-center justify-start">
                    @foreach($category->children as $child)
                        <li class="mr-4 font-normal text-xs hover:bg-gray-200 rounded-[4px] cursor-pointer p-1">
                            <a href="items?filters[categoryIds][]={{$child->id}}">{{$child->name}}</a>
                        </li>
                    @endforeach
                </ul>
